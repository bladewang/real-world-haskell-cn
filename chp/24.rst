...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The familiar seq function evaluates an expression to what we call head normal form (abbreviated HNF). It stops once it reaches the outermost constructor (the “head”). This is distinct from normal form (NF), in which an expression is completely evaluated. No comments

You will also hear Haskell programmers refer to weak head normal form (WHNF). For normal data, weak head normal form is the same as head normal form. The difference only arises for functions, and is too abstruse to concern us here. 2 comments

排序
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
下面是一个普通的 ``Haskell`` 函数，其使用分治法对一个列表进行排序：
